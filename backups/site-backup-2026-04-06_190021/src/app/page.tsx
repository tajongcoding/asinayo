...
export default function Home() {
  ...
  const faqPreviewItems = [
    { question: '울산 청년월세 지원은 어디서 신청하나요?', category: '경제' },
    { question: '울산 대형폐기물 신고는 어떻게 하나요?', category: '생활' },
    { question: '울산 야간약국은 어디서 찾을 수 있나요?', category: '생활' },
  ];
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {topKeywords.map((keyword) => (
                <Link
                  key={keyword.label}
                  href={keyword.href}
                  className="rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-[12px] md:text-[13px] font-bold text-white hover:border-[#C9A857] hover:text-[#C9A857] transition-colors"
                >
                  #{keyword.label}
                </Link>
              ))}
            ...
            {featuredVisuals.map((item, index) => (
              ...
              >
                ...
                <div className="absolute left-0 right-0 bottom-0 p-3.5">
                  <p className={`inline-flex rounded-full px-2.5 py-1 text-[12px] font-bold mb-2 ${item.badgeClass}`}>
                    {item.categoryLabel}
                  </p>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {siteBenefits.map((item) => (
            <div key={item.title} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
              <div className="text-[13px] font-black tracking-widest text-[#C9A857] uppercase mb-2">핵심 안내</div>
              <h3 className="text-[18px] md:text-[20px] font-black text-[#0F1A2B] mb-2 break-keep">{item.title}</h3>
              <p className="text-slate-600 text-[15px] leading-relaxed break-keep">{item.desc}</p>
            </div>
          ))}
        </section>
