          ...
          {menuItems.map((item, idx) => {
            ...
            return (
              <div key={idx} className="relative group/menu h-full flex items-center">
                <Link
                  href={item.path}
                  className={`relative text-[16px] md:text-[18px] font-extrabold transition-all duration-300 group/item whitespace-nowrap flex items-center gap-1.5 py-6 ${item.highlight
                    ? isActive
                      ? 'text-[#FFE08A]'
